...
const validBody = {
  ...
  href: "/investigations/operation-example",
  period: "2020-2021",
  periodLabel: "2020 - 2021",
  title: "Operation Example",
  role: "Lead investigator",
  text: "A summary of the investigation.",
};
...
describe("investigations routes", () => {
  ...
  describe("GET /api/investigations", () => {
    it("returns data without requiring auth", async () => {
      state.selectResult = [{ id: 1, ...validBody, tags: [], featured: false, sortOrder: 0 }];
  ...
  describe("POST /api/investigations", () => {
    ...
    it("creates a record when the admin token is valid", async () => {
      const created = { id: 1, ...validBody, tags: [], featured: false, sortOrder: 0 };
  ...
  describe("PATCH /api/investigations/:id", () => {
    it("rejects requests without a valid admin token", async () => {
      const res = await request(app).patch("/api/investigations/1").send({ title: "New" });
    ...
    it("returns 400 for a non-numeric id", async () => {
      const res = await request(app)
        .patch("/api/investigations/not-a-number")
        .set("X-Admin-Token", ADMIN_TOKEN)
        .send({ title: "New" });
    ...
    it("updates and returns the record when found", async () => {
      const updated = { id: 1, ...validBody, title: "New", tags: [], featured: false, sortOrder: 0 };
  ...
  describe("DELETE /api/investigations/:id", () => {
    it("rejects requests without a valid admin token", async () => {
      const res = await request(app).delete("/api/investigations/1");
    ...
    it("returns 400 for a non-numeric id", async () => {
      const res = await request(app)
        .delete("/api/investigations/not-a-number")
        .set("X-Admin-Token", ADMIN_TOKEN);
